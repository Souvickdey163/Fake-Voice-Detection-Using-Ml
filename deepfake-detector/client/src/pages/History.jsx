...
import { History as HistoryIcon, Clock, HardDrive, ShieldAlert, ShieldCheck } from 'lucide-react';
    ...
    <div className="max-w-6xl mx-auto py-8 px-4 sm:px-6 lg:px-8 animate-fade-in-up">
      <div className="flex items-center space-x-4 mb-8">
        ...
        <div>
          ...
          <p className="text-gray-400 mt-1">Review your previously analyzed audio files.</p>
        </div>
      </div>
      ...
      <div className="glass-panel overflow-hidden">
        {loading ? (
          ...
          <div className="p-10 text-center flex flex-col items-center justify-center h-64">
            ...
            <p className="text-gray-500 mt-2">Upload and analyze audio to see results here.</p>
          </div>
        ) : (
          <div className="overflow-x-auto">
            <table className="min-w-full divide-y divide-gray-700/50 text-left">
              <thead className="bg-gray-800/40">
                <tr>
                  <th className="px-6 py-4 text-xs font-medium text-gray-400 uppercase tracking-wider">File Details</th>
                  <th className="px-6 py-4 text-xs font-medium text-gray-400 uppercase tracking-wider">Prediction</th>
                  <th className="px-6 py-4 text-xs font-medium text-gray-400 uppercase tracking-wider">Confidence</th>
                  <th className="px-6 py-4 text-xs font-medium text-gray-400 uppercase tracking-wider">Date & Time</th>
                </tr>
              </thead>
                  ...
                  <tr key={record.id} className="hover:bg-gray-800/20 transition-colors">
                    <td className="px-6 py-5 whitespace-nowrap">
                      <div className="flex items-center">
                        ...
                        <div className="text-sm font-medium text-white max-w-[200px] sm:max-w-xs truncate" title={record.filename}>
                          {record.filename}
                        </div>
                      </div>
                    </td>
                    <td className="px-6 py-5 whitespace-nowrap">
                      <span className={`inline-flex items-center space-x-1.5 px-3 py-1 rounded-full text-xs font-medium border ${
                        ...
                        {record.prediction === 'Bonafide (Real)' ? <ShieldCheck className="w-3.5 h-3.5" /> : <ShieldAlert className="w-3.5 h-3.5" />}
                    ...
                    <td className="px-6 py-5 whitespace-nowrap">
                      <div className="flex items-center max-w-[120px]">
                        <div className="w-full bg-gray-700 rounded-full h-2 mr-3">
                          <div 
                            className={`h-2 rounded-full ${record.prediction === 'Bonafide (Real)' ? 'bg-green-500' : 'bg-red-500'}`} 
                            style={{ width: `${record.confidence}%` }}
                          ></div>
                        </div>
                        <span className="text-sm text-gray-300 font-mono">{record.confidence.toFixed(1)}%</span>
                      </div>
                    </td>
